...
class Lexer:
    ...
    def read_string(self) -> str:
        """Read a double-quoted string (supports triple quotes for docstrings)."""
        ...
        if self.peek() == '"' and self.peek(1) == '"':
            ...
            return self._read_triple_quoted_string()
    ...
    def _read_triple_quoted_string(self) -> str:
...
class Parser:
    ...
    def _nud_string(self, token: Token) -> StringLiteral:
        """Parse a string literal."""
        return StringLiteral(token.value)

    def _nud_number(self, token: Token) -> IntegerLiteral:
    ...
    def _nud_grouping(self, token: Token) -> Expression:
    ...
    def led(self, left: Expression, token: Token) -> Expression:
        """
    ...
    def _led_type_annotation(self, left: Expression, token: Token) -> Tuple[Expression, Type]:
        """
        ...
        if isinstance(type_expr, Identifier):
            return (left, Type(type_expr.name))
        else:
            self.error("Expected type name after ::")

    def _led_arrow(self, left: Expression, token: Token) -> Any:
    ...
    def _led_call(self, left: Expression, token: Token) -> Any:
    ...
    def _led_assignment(self, left: Expression, token: Token) -> Any:
        """Parse assignment: left = right."""
        right = self.expr(BP_ASSIGNMENT - 1)  # Right associative
        return {"assign": left, "value": right}

    def _led_with(self, left: Expression, token: Token) -> Any:
        """Parse with clause for LLM calls."""
        right = self.expr(BP_WITH)
    ...
    def expr(self, rbp: int = BP_NONE) -> Expression:
        """
    ...
    def parse_statement(self) -> Optional[Statement]:
        ...
        if self.match(TokenType.DEF):
            ...
            self.next()  # consume pass
            return None  # pass is a no-op
        elif self.match(TokenType.EOF):
            return None
        else:
            # Try to parse as expression statement
            expr = self.parse_expression_or_llm_call()
            if expr:
    ...
    def parse_return(self) -> ReturnStmt:
        ...
        if self.match(TokenType.NEWLINE, TokenType.DEDENT, TokenType.EOF):
            return ReturnStmt(value=Identifier("None"))
        else:
            value = self.expr()
            return ReturnStmt(value=value)

    def parse_expression_or_llm_call(self) -> Optional[Expression]:
        """
        ...
        elif self.match(TokenType.IDENTIFIER, TokenType.STRING, TokenType.NUMBER):
            return self.expr()
        else:
            return None
...
def main():
    ...
    try:
        ...
        for func in ast.functions:
            params_str = ", ".join(f"{p.name} :: {p.type_.name}" for p in func.params)
            return_type_str = func.return_type.name if func.return_type else "None"
            print(f"  - {func.name}({params_str}) -> {return_type_str}")
