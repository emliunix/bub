...
def equals_ignore_location(a: object, b: object) -> bool:
    """Compare two AST nodes for structural equality, ignoring location.
    ...
    # Compare dataclass fields, ignoring location/source_loc fields
    a_fields = getattr(a, "__dataclass_fields__")
    for field_name in a_fields:
        if field_name in ("location", "source_loc"):
            continue
