...
class MessageBus:
    ...
    async def send_message(self, to: str, payload: dict[str, Any]) -> Any:
        """Send a message to a destination (bridges to publish_outbound for now)."""
        # Extract channel and chat_id from 'to' address
        # Format: "channel:chat_id" (e.g., "telegram:123456")
        if ":" in to:
            channel, chat_id = to.split(":", 1)
        else:
            channel = "unknown"
            chat_id = to
        ...
        await self.publish_outbound(message)
