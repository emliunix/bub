...
✅ wsbus.py cleaned and regenerated
✅ All callers updated
✅ Type checking passes
...
### Status
✅ E2E test passing (spawn → message → response working)
🔄 Investigating systemd unit conflicts on respawn
...
**Result:** 121 lines removed (884 → 760 lines)
...
```
Telegram → AgentBusClient.publish_inbound() → constructs tg_message payload
```

**After:** Protocol layer is generic, callers construct payloads
```
Telegram → create_tg_message_payload() → AgentBusClient.send_message()
```

This follows the pattern: **Encoding logic in messages.py, usage in channel adapters**.

### Status
✅ Telegram-specific methods removed from wsbus.py
✅ Message construction helpers added to messages.py
✅ All callers updated to use new API
✅ Old MessageBus class deleted
✅ Tests updated
✅ Type checking passes (pre-existing errors in wsbus.py unrelated to this refactor)
...
- **Added section 8**: Message Flow sequence diagram with mermaid
  - Complete flow: sendMessage → processMessage → response aggregation
...
- **Moved authorization matrix**: Section 9 → Appendix A (future feature)
...
- **Renumbered sections**: 8-17 → 8-18 (new section 8 inserted)
...
- Mermaid sequence diagram showing sendMessage → processMessage → acks flow
...
### Status
✅ agent-protocol.md updated with sequence diagram
✅ agent-messages.md created with full message type documentation
✅ protocol.py SendMessageParams fixed to include `from` field
✅ All examples consistent between docs and implementation
✅ Committed (bdec880)
...
```
src/bub/
├── bus/                    # Bus transport layer
│   ├── __init__.py        # Exports bus classes
│   ├── bus.py             # WebSocket bus implementation
│   └── protocol.py        # Bus protocol types
├── message/               # Message definitions
│   ├── __init__.py        # Exports message types
│   └── messages.py        # Message payloads & helpers
└── rpc/                   # Generic RPC framework
    ├── __init__.py        # Exports JSONRPC framework
    ├── framework.py       # JSON-RPC 2.0 implementation
    └── types.py           # Generic RPC types
```

### Changes Made

**Files moved:**
- `src/bub/channels/wsbus.py` → `src/bub/bus/bus.py`
- `src/bub/rpc/protocol.py` → `src/bub/bus/protocol.py`
- `src/bub/rpc/messages.py` → `src/bub/message/messages.py`
...
✅ Type checking passes (pre-existing errors in bus.py unchanged)
...
### Status
✅ ActivityLogWriter moved to `bub/bus/log.py`
✅ SQLAlchemy ORM model implemented
✅ Queue-based async pattern working
✅ Batched writes implemented (10 entries / 100ms)
✅ WAL mode enabled for SQLite
✅ Type checking passes
✅ Exported from `bub.bus` package
...
- **Renamed `publish` → `_dispatch_process_message()`**: Better describes what it does
...
```
Sender Peer → AgentBusClient.send_message() 
  → AgentBusClientApi.send_message()
  → JSON-RPC → Bus Server
  → AgentConnection.send_message()
  → AgentBusServer._dispatch_process_message()
  → _process_message_for_peer() [for each match]
  → AgentBusServerApi.process_message()
  → JSON-RPC → Recipient Peer
  → AgentBusClient.process_message()
  → Handler dispatch
  → ProcessMessageResult (with retry fields)
  → [Stripped to MessageAck]
  → SendMessageResult with MessageAck[]
```

### Status
✅ ProcessMessageResult fields fixed
✅ from_ field properly propagated
✅ MessageAck type created
✅ SendMessageResult.acks uses MessageAck
✅ publish renamed to _dispatch_process_message
✅ conn_id removed from sender fallback
✅ Documentation updated
✅ Protocol and implementation now consistent

---

## Integration Test for Bus Protocol

### Goal
Create an integration test to verify the bus protocol works correctly with multiple clients.

### Test Scenario
- Client A sends messages to B and C
- B and C receive the messages
- B and C send responses back to A
- A receives both responses

### Files Created
- `scripts/test_bus_integration_3clients.py` - Main integration test
- `scripts/test_bus_concurrent.py` - Concurrent handling test

### Issues Fixed During Testing
1. **Missing SQLAlchemy dependency** - Added `sqlalchemy>=2.0.0` to pyproject.toml
2. **WAL event handler signature** - Fixed `_enable_wal()` to accept `(dbapi_connection, connection_record)`
3. **Port conflicts** - Used dynamic port allocation (port=0) to avoid conflicts with existing bus
4. **Message type whitelist** - Bus only allows specific message types, used `agent_event` for testing
